...
| `scripts/combat/player_weapon.gd` (`PlayerWeapon`, CODE_READY F6-03) | `PlayerShip/Weapon` (wired in `player_ship.tscn`) | **Scene references** (required): `muzzle` → `../Muzzle`, `familiar_anchors` → `../FamiliarAnchors` (with `Left` and `Right` markers), `camera_rig` → `../CameraRig`, `familiar_scene` (dev `scenes/dev/familiar.tscn` until D-02; a `Node3D` root with no `CollisionObject3D`). **Shots**: `shot_speed` 60, `shot_lifetime` 1.2, `shot_radius` 0.15, `shot_damage` 1, `familiar_shot_damage` 1. **Cadence and Aim Assist**: `main_interval` 0.1, `familiar_interval_level_2` 0.25, `familiar_interval_level_3` 0.15, `main_assist_degrees` 10, `familiar_assist_degrees_level_2` 10, `familiar_assist_degrees_level_3` 20. All Claude's proposals, yours to tune. **Bomb** (F7-02): `bomb_radius` 10.0, `bomb_damage` 20, `bomb_visual_scene` (dev `scenes/dev/bomb_blast.tscn`, a `BombBlast` root, until D-02) | `setup(combat_state, projectile_system, targeting)`, `set_fire_enabled(enabled)`. Holding `fire` spawns player Projectiles through the `WeaponModel` cadence from `Muzzle` and, from Power Level 2, the two Familiar anchors, rotated by the camera yaw, toward the view's center at the lock's depth, bent onto the locked target's `HitVolume` inside each shot's Aim Assist cone. Two top-level Familiars follow the anchors with a small orbit. The `bomb` button is tracked from events and fed to `CombatState.update_bomb_input()` once per tick. It ticks at physics priority 50, after the actors and before the ProjectileSystem. Contract in [engineering/weapon-rendering.md](engineering/weapon-rendering.md) |
...
| `scripts/progression/stage_director.gd` (`StageDirector`, CODE_READY F10-01, attached to Stage 1's `Stage` by Claude) | Stage | Nothing on Stage 1: Claude sets `stage_definition` (`content/stages/stage_01/stage_01.tres`), `actor_scenes` and `enemy_definitions` by Wave kind (`spirit`, `sentry`, and `lantern_guardian` on the dev Sentry until F12-03), `power_pickup_scene`, `shield_pickup_scene` and `reward_spread` (1.5). Keep the `Encounters/<ID>/{EntryVolume,ExitVolume,Spawns,RewardOrigin,ShieldPickup}` and `RuntimeActors` names; `monitoring` stays off in the file | `check_setup()` before loading; `setup(...)` arms the volumes (deferred `body_entered`) and the `EncounterMachine`; `start_attempt(seed)` begins S1-01; spawns Waves and reward Pickups under `RuntimeActors`, scores each defeat once, relays `threat_reported`, `pickup_accepted` and `stage_cleared`; `get_active_encounter_bounds()`. Gates, Checkpoints and PortalLinks in F10-02, Retry in F10-03. Contract in [engineering/stage-director.md](engineering/stage-director.md) |
...
| `scripts/ui/interface.gd` (`Interface`, CODE_READY F2-02) | `Main/Interface` | Nothing: the node lives in the Claude-owned `scenes/main.tscn`, where `menu_scenes` holds the eight menus and `hud_scene` the HUD | Instances the eight menus and the HUD once, drives them through `ScreenRouter` (show, hide, initial focus on entry, remembered focus on return), resolves Back: `ui_cancel` and Back buttons return to the caller, request `resume` on Pause and `back_refused` where there is nowhere to go, and are left to the Session over gameplay. Signal `action_requested(action, payload)` for every other menu action; `show_home`, `show_screen`, `push_overlay`, `back`, `current_screen`, `is_gameplay_covered`, `get_hud`. Contract in [engineering/menus-session.md](engineering/menus-session.md) |
| `scripts/ui/menu_controller.gd` (`MenuController`, CODE_READY F2-02) | Menu scene root Control (all eight, already attached) | Nothing in the Inspector. Keep the root names and every Section 14 button path, plus `Layout/Score`, `Layout/RetryLocation`, `Layout/Heading` and `Layout/NavigationHint`: they are load-bearing, and renaming one needs a matching code change announced in the handoff log. Tree order sets the initial focus | Identifies its screen from the root name, connects the registry buttons by path to `action_requested(action, payload)`, focuses the first focusable control on entry or the remembered one on return, writes Pause's score and Graze, Defeat's retry location and Results' mode (Continue or Replay or neither, heading, focus loop), and hides the keyboard footer while a gamepad is in use |
...
| Boss phase changed | Boss → presentation/HUD | Phase index, attack display name | Clear old phase threats and show a brief cue. Since F4-03 the HUD side is a direct call from the owner (F12-03 wires it): `Hud.show_boss(display_name, phase_count)` on entry, `set_phase_health(phase_index, ratio)` per change, `show_attack_cue(attack_name, seconds)` on a transition, `hide_boss()` on defeat or unload |
...
| Player defeated | Player → session | Active stage and attempt context | Stop combat and show Retry/Main Menu. Since F7-01: `CombatState.defeated` (once per life) → `GameSession._on_player_defeated()`, which pauses the tree, Active Time, the controls and the core, and pushes the Defeat overlay with `{"checkpoint": ""}` (`Início da fase`); `retry` restarts the stage until F10-03 |
| Stage completed | Stage director → session | Stage ID and committed result. Since F10-01: `StageDirector.stage_cleared` (no payload; the Session knows the stage), after the machine's last `encounter_completed` | Campaign continuation or isolated-stage results. The Session connects it `CONNECT_DEFERRED` to `RunState.complete_stage()`, whose result returns to the menu until F11-01 |
| Pause requested | Input/menu → session | Since F2-04: the `pause` action (Escape, gamepad Start) reaches `GameSession._unhandled_input` over the HUD or Pause; Continuar and `ui_cancel` on Pause arrive as `resume` from `Interface` | Freeze world/timers, preserve menu processing. Done by `GameSession`: tree paused, `RunState` paused, ship controls off, Pause overlay; ignored under Options opened from Pause |
...
| Main composition and session | Claude (`scenes/main.tscn`, F0-03) | Claude | CODE_READY | Composition root instanced headless by `tests/scene/test_main_contract.gd`; since F2-04 `GameSession` starts a Campaign or Direct Stage 1 or 2 from the menus, spawns the ship at `PlayerStart` inside the stage's Flight Volume, pauses (tree, Active Time, controls), resumes, restarts, returns to the menu and quits: 20 flow tests in `tests/scene/test_game_session_flow.gd`, sixteen mutants caught, and the scripted keyboard and gamepad flight pass in [validation/menus.md](validation/menus.md) (`menus-flight.png`, `menus-pause-return.png`). Stage completion, defeat and results are F10 and F11; the physical keyboard and pad pass is owed |
...
| Common enemies and miniboss | Astra | Claude | CODE_READY (common enemies) | F9-01 `EnemyModel` and F9-02 `EnemyActor`: dev Spirit and Sentry prefabs (`scenes/dev/`, with Astra's `spirit_lume` and `sentry_lantern` visuals) spawn, pulse through Anticipation, fire their patterns, take player shots and the Bomb, warn off-screen and report one defeat in the arena harness; scripted runs and `enemies-arena.png` in [validation/enemies.md](validation/enemies.md). Final `scenes/enemies/spirit.tscn` and `sentry.tscn` and the D-05 tuning are Astra's; the miniboss is F12-06 |
...
| Stage 1 progression | Astra | Claude | CODE_READY (F10-01) | Since F10-01 Stage 1's Encounters play: S1-01 on entry, Waves spawn and fire, defeats score once, rewards drop, and the route stops at the closed `Gate_S1_02`; Gates, Checkpoints and PortalLinks arrive with F10-02, Retry with F10-03 |
