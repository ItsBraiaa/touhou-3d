...
- `Main/Audio` — Node, `process_mode = ALWAYS`, with `scripts/audio/audio_controller.gd` (`AudioController`, F13-03) attached in `scenes/main.tscn`; `Main`'s export `audio` points at it.
...
| --- | --- | --- | --- | --- |
| Foundation and conventions | Claude | Claude | CODE_READY | F0 done: repository hygiene, `tools/godot.*` and `tools/test.*`, project configuration, input actions, audio buses, export preset, and this contract. Since F14-01 the Windows release build is exported and runs outside the editor: D3D12 Forward+, 1280 × 720, 59 to 60 FPS in both boss fights, no fallback ([validation/export.md](validation/export.md)); [engineering/ROADMAP.md](engineering/ROADMAP.md) |
...
| Storm Guardian | Astra | Claude | CODE_READY F12-07 | The D-04 `storm_guardian.tscn` prefab now carries `BossController` and Flying_Idle, Punch, Yes, Death. Its three named Phases in S2-07 end Stage 2 and the Campaign after the final defeat; D-06 pass 3 owns numeric tuning. |
...
| Stage 2 progression | Astra | Astra (sol) | CODE_READY F12-07 | Director, five Gates, three Seals, CP2-A/CP2-B, and both real bosses wired. S2-07's final boss defeat completes Stage 2. |
...
| Audio integration | Astra: selection/mix | Claude: runtime | CODE_READY (F13-03) | D-01's 17 non-looping Kenney effects play through `Main/Audio`'s `AudioController`, one sound per producer emission within the limiter, also after Restart and Retry, and nothing after an unload; no music ships. Driven headless and windowed in [validation/audio.md](validation/audio.md); the human listening pass is owed (F14-02) |
