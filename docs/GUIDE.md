...
| --- | --- | --- | --- |
| `scripts/session/game_session.gd` | Main | Nothing yet: `world_root: Node3D`, `projectile_root: Node3D`, `interface: Interface` (typed since F2-02), `audio: Node` are set in the Claude-owned `scenes/main.tscn`; stage scene references come with F2/F11 | Validates the four exports and opens the main menu through `interface.show_home` (F0-03, F2-02); menu actions, campaign/direct-stage lifecycle, pause, results and transitions (F2-04, F11) |
...
| Main composition and session | Claude (`scenes/main.tscn`, F0-03) | Claude | CODE_READY | Composition root instanced headless by `tests/scene/test_main_contract.gd`; main menu shown at startup through `Interface` (F2-02); menu actions, stage start and pause pending (F2-04) |
...
| Menus/options | Astra | Claude | SCENE_READY | Eight menu scenes rendered and checked; Section 14. Navigation wired by F2-02 — registry buttons, focus on entry and return, Back, runtime text, footer — with 26 scene tests and a scripted keyboard and gamepad pass in [validation/menus.md](validation/menus.md); session actions (F2-04) and settings (F3) pending, physical-device pass owed |
