...
- **No new tests, ever (the user's rule, 2026-09-23).**
  ...
  - The only automated gate is `tools/lane.ps1 land`: the existing suite, a 300-frame headless boot of the main scene, and `tools/check_resources.gd`. That check compiles every `.gd` under `scripts/`, `tools/` and `tests/` (the runner silently skips a test file that fails to compile), loads every `.tres` and `.tscn` under `content/` and `scenes/`, and runs each Definition's `validate()`. A red run, a `SCRIPT ERROR`, a parse error or a failed script load stops the landing, and so does any `ERROR:` line during the boot. The suite prints `ERROR:` lines on purpose, from tests of loud setup errors.
