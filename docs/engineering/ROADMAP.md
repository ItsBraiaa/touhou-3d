...
| F14-02 | **Credits gaps** found by the acceptance walk ([validation/acceptance.md](../validation/acceptance.md) "Credits coverage"). Add `ASSET_CREDITS.md` entries for the five original D-02 materials in `assets/combat/` (`player_shot`, `hostile_shot`, `bomb_ring`, `power_gold`, `shield_blue`) and for the original `assets/environment/stage_01/gate_veil.gdshader`; and add the Quaternius enemy/boss models to the in-game Créditos text (`scenes/ui/credits.tscn`), which credits the ship and the sound effects but not the models. The files themselves need no change. |
