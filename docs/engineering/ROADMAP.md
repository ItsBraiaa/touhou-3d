...
| | | | 05 audio-endings-polish (no ticket file; user request) | integration | no | path | done (2026-09-24: a boss's defeat plays only `boss_defeated`; `stage_cleared` waits out the 1.48 s bell (`play_event_after`); Sair and the window close silence the audio before quitting, so no "resources still in use at exit"; headless S1-07 clear and F13-04's 64-check driver green; no tests per the sprint rule) |
...
| F16-03 rebind-workflow-and-prompts | Claude / trunk | F16-01, F16-02, F16-08 | done 2026-09-24 (`ControlsScreen`: catalog rows with glyphs, full focus loop, release-gated capture with drift/trigger/chord rules, Trocar/Substituir/Cancelar, draft Aplicar with save-failure text and 10 s confirmation rollback, Dirty dialog, Câmera tab and Ícones; menu footers from the live bindings and prompt family; settings.md "F16 capture workflow and prompts"; device walk pending F16-07) |
...
| F16-05 invulnerable-lateral-dash | Claude / rescue | F16-00 (part 2: F16-01) | done (2026-09-24, both parts: `DashModel`, a collision-stopped camera-lateral burst in `PlayerController`, the synchronous `dash_started` → `CombatState.grant_invulnerability` wiring with the tick order proven, `CombatState.TIME_EPSILON` ending windows on their last tick, the `DashVisual` and `DashCooldown` ("Impulso") components wired; player-flight.md and combat-hud.md F16-05 sections; device feel and look owed to F16-07) |
| F16-06 session-camera-and-controls-integration | Claude / trunk | F16-03, F16-04, F16-05 | done 2026-09-24 (`GameSession`: all six camera values to every ship at spawn and on change, over Pause too; sole owner of `Input.mouse_mode`, captured only while flying in Mouse mode, shown on every menu, overlay, unload and focus loss, with the capture warp dropped; focus loss pauses a stage in play and focus-in resumes nothing; `camera_recenter` only in flight; `PlayerController` resume guard so a press that resumes play never dashes; GUIDE, PLANEJAMENTO, settings.md and player-flight.md "F16 Session integration"; 16-step human walkthrough owed to F16-07) |
| F16-07 controls-visual-and-device-acceptance | Astra / sol; Claude engineering fixes | F16-06 | blocked 2026-09-24 (integrated source inspected; visible Câmera slider labels added; user reports no physical-device checks; interactive Controls, glyph, cooldown and dash acceptance remains not verified in validation record) |
| F16-09 targeting-ignores-the-resume-press | OpenCode / oc-a | F16-06 | done (2026-09-24: `Targeting` gains the dash's release latch, `_input_armed` plus public `require_release()`, which `PlayerController.set_controls_enabled(true)` calls beside the dash guard; the resume press can reach neither dash nor targeting; player-flight.md "Targeting contract" and menus-session.md Open issues updated; no tests per the sprint rule; the B-resume device check belongs to F16-07) |
| F16-10 spec-api-list-matches-shipped-code | OpenCode / oc-b | F16-06 | done (2026-09-24: the spec's "Proposed APIs" block is now "Shipped APIs (F16-02 to F16-08)", listing every F16 public member with `# was:` notes for corrected and added ones; the added F16-05 `DashModel`/`PlayerController` members and the new `BindingLabels`/`InputDeviceState` API are included; heading and following paragraph updated; `spec.md` only, no code, scene or test; no tests per the sprint rule) |
| F16-11 target-lock-handoff-on-defeat | Claude / path | F16-04, F16-09 | done (2026-09-24: the lock watches only its own target's `defeated`, hands off next tick to `TargetSelector.select_successor` (visible, in range, nearest center), else releases and recenters through `lock_lost_to_defeat`; no recenter while a boss's Death clip plays; unrelated deaths, manual unlock, range loss unchanged; headless in-game driver, no tests) |
...
| F13 | **D-01** delivered; since F13-04 your revised selection (`sound_effects/README.md`) is the shipped mix, from `Main/Audio` (`event_streams`, `event_volume_db`, `max_voices` 8 in `scenes/main.tscn`) and `EVENT_RULES`. Music ships since 2026-09-24: the five `music/` tracks, copied to `assets/audio/music/` and wired in `Main/Audio.music_tracks` per PLANEJAMENTO Section 9. Open: the listening pass (human pass, F14-02). To retune a sound's interval, voices, priority or volume, or swap a file, send Claude a new catalogue row; `checkpoint_activated` now also plays a sound (the arch glow is still yours). |
...
Both-stage closed gate clarity: `.scratch/gate-clarity/issues/01-closed-gate-veils.md` — **done** (denser blue mist in Stage 1 arches; wind-folded violet storm veils in Stage 2 arches; visual collision and gate wiring unchanged).
