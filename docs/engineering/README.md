...
| [player-flight.md](player-flight.md) | Module doc for player flight: the `FlightModel` Rules Core — velocity from input axes and camera yaw, Flight Volume clamping, edge proximity, and the visual bank angle — and the `PlayerController` adapter that reads the input, moves the body and banks the visuals, with the dev arena harness. |
