...
Engineering follow-up for Claude (trunk): F16-06's `targeting.gd` concern remains open by code reading, not a witnessed failure. Reproduction to exercise: bind `lock_target` or `next_target` to the controller Back input, Pause, press Back to resume, and inspect whether Targeting locks or switches on the first unpaused tick. If reproduced, apply the same release-before-arming rule used for dash. This is outside F16-07's Files boundary; no Astra script edit was made.
