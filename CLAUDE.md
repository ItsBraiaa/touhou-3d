...
0. **Sprint to 2026-09-24:** lanes `trunk` and `path` (Claude Opus; `rescue` on demand), `oc-a` and `oc-b` (OpenCode, model chosen per ticket by its `Model:` line), `sol` (Astra on GPT Sol) and, optionally, `terra` (Codex Terra). Each lane has its own git worktree and `lane/<name>` branch. Never work in the primary tree. Read [docs/engineering/SPRINT.md](docs/engineering/SPRINT.md) first; it gives your lane's queue, the worktree loop (`tools/lane.ps1 sync`, `status`, `land`) and your kickoff prompt.
