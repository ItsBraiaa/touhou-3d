...
Part 1 implementation is committed as `combat: add WeaponModel core (F6-03 part 1)`: the Node-free `WeaponModel` emits cadence-based main/Familiar shots, counts Familiars by Power Level, and provides normalized static Aim Assist direction. No tests were added per the sprint rule. **Land blocked:** `powershell -NoProfile -ExecutionPolicy Bypass -File tools\lane.ps1 land` stopped with `lane: commit your work before landing. Uncommitted: ?? scripts/definitions/enemy_definition.gd.uid; ?? scripts/enemies/enemy_model.gd.uid`. Hypothesis: the earlier F9-01 Godot import generated these script UID files after its successful gate preflight, and the delayed artifacts surfaced at the next preflight. This repeats the lane gate's uncommitted-UID failure after previous UID fixes; stop here for escalation. F6-03 remains todo for trunk's part 2 (`PlayerWeapon` and scene integration).
