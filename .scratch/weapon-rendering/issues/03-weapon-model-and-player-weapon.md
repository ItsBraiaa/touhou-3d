...
Part 1 landed as `combat: add WeaponModel core (F6-03 part 1)`: the Node-free `WeaponModel` emits cadence-based main/Familiar shots, counts Familiars by Power Level, and provides normalized static Aim Assist direction. No tests were added per the sprint rule. F6-03 remains todo for trunk's part 2 (`PlayerWeapon` and scene integration).
