...
0. **Sprint to 2026-09-24:** four lanes (Claude Opus `trunk`, Astra on GPT Sol `sol`, OpenCode GLM `glm-a` and `glm-b`). Each lane has its own git worktree and `lane/<name>` branch. Never work in the primary tree. Read [docs/engineering/SPRINT.md](docs/engineering/SPRINT.md) first; it gives your lane's queue, the worktree loop (`tools/lane.ps1 sync`, `status`, `land`) and your kickoff prompt. Claude skill names in kickoff prompts (`/mattpocock-skills:tdd`, `/run`) are Claude-only; follow their intent (test-first; verify headless).
